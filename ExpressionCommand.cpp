...
ExpressionCommand::ExpressionCommand(Command *c,
                                     vector<string>::iterator &script, SymbolTable* table,CreateExpression *create) {
    this->command = c;
    this->script = script;
    this->symbolTable = table;
    this->createExpression=create;
}
...
double ExpressionCommand::calculate(map<string, double> &assignment) {
    ...
    if (((*script) == "openDateServer") || ((*script) == "sleep")) {
        ...
        countSteps++;
        expression = (*script);
        newExpression = this->createExpression->convertToExpression(expression);
        resultExpression = (int) newExpression->calculate(assignment);
        newVector.push_back(to_string(resultExpression));
        vector<string>::iterator itr;
        itr=newVector.begin();
        ...
        newExpression = this->createExpression->convertToExpression(expressin);
